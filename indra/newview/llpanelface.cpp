...
void LLPanelFace::onPasteTexture()
{
    ...
    for (; iter != end; ++iter)
    {
        ...
        {
            bool full_perm = te_data["te"].has("imgitemfullperm") && te_data["te"]["imgitemfullperm"].asBoolean();
            full_perm_object &= full_perm;
            if (!full_perm)
            {
                if (te_data["te"].has("imgitemid"))
                {
                    LLUUID item_id = te_data["te"]["imgitemid"].asUUID();
                    if (item_id.notNull())
                    {
                        LLViewerInventoryItem* itemp = gInventory.getItem(item_id);
                        if (!itemp)
...
void get_item_and_permissions(const LLUUID &id, LLViewerInventoryItem*& itemp, bool& full_perm, bool& from_library, const LLSD &data, const std::string &modifier)
{
    full_perm = data.has(modifier + "itemfullperm") && data.get(modifier + "itemfullperm").asBoolean();
    from_library = data.has(modifier + "fromlibrary") && data.get(modifier + "fromlibrary").asBoolean();
    LLViewerInventoryItem* itemp_res = NULL;

    if (data.has(modifier + "itemid"))
    {
        LLUUID item_id = data.get(modifier + "itemid").asUUID();
        if (item_id.notNull())
        {
            LLViewerInventoryItem* itemp = gInventory.getItem(item_id);
    ...
    if (!itemp_res && !full_perm)
    {
        ...
        LL_INFOS() << "Item " << data.get(modifier + "itemid").asUUID() << " no longer in inventory." << LL_ENDL;
